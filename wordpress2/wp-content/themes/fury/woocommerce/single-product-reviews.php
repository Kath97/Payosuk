	...
	<div id="comments">
		<h4 class="woocommerce-Reviews-title">
			...
			if ( $count && wc_review_ratings_enabled() ) {
				...
				$reviews_title = sprintf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'fury' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
				echo apply_filters( 'woocommerce_reviews_title', $reviews_title, $count, $product ); // WPCS: XSS ok.
			} else {
				esc_html_e( 'Reviews', 'fury' );
			}
			?>
        </h4>
			...
			<div id="review_form">
				<?php
					...
					if ( wc_review_ratings_enabled() ) {
						$comment_form['comment_field'] = '<div class="col-12"><label for="rating">' . esc_html__( 'Your rating', 'fury' ) . '</label><select name="rating" id="rating" required>
					...
					$comment_form['comment_field'] .= '<div class="col-12"><label for="comment">' . esc_html__( 'Your review', 'fury' ) . ' <span class="required">*</span></label><textarea id="comment" class="form-control form-control-rounded" name="comment" cols="45" rows="8" required></textarea></div>';
