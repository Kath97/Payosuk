<?php
						...
						<div class="copyright text-center">
							<?php if( get_theme_mod('density-footer_title') ) : ?>
								...
								<?php else : 
									...
									printf( esc_html__( ' %1$sPowered by %2$s%3$s', 'density-vertical' ), '<span>', '<a href="'. esc_url( __( 'https://wordpress.org/', 'density-vertical' ) ) .'" target="_blank">"'. esc_html__('WordPress','density-vertical').'".</a>', '</span>' );
									/* translators: 1: poweredby, 2: link, 3: span tag closed  */
									printf( esc_html__( ' Theme: density by: %1$sDesign By %2$s%3$s', 'density-vertical' ), '<span>', '<a href="'. esc_url( __( 'http://wordpress.org/', 'density-vertical' ) ) .'" target="_blank">"'. esc_html__('wpalter','density-vertical').'"</a>', '</span>' );
							endif;  ?>
						</div>
					</div>
				</div>
	...
	<a class="density_tm_totop right-fixed">
		...
		<span class="name"><?php echo esc_html__( 'To Top', 'density-vertical' ); ?></span>
	</a>
	<!-- /TO TOP -->
</div>
</div>
